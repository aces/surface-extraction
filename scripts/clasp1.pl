...
my $usage = <<USAGE;
usage: $ProgramName [options] classified.mnc final.mnc mask.mnc
...
if ( !defined $Mask ) {
    ...
    Spawn(["minccalc", "-expression", 'out=1;', $input_volume, $Filled]);
    Spawn(["surface_mask2", "-binary_mask", $Filled, $Mask, "${TmpDir}/cortical_mask.mnc"]);
    Spawn(["rm", "-f", $Filled]);
    $Mask = "${TmpDir}/cortical_mask.mnc";
    Spawn(["dilate_volume", $Mask, "${TmpDir}/cortical_mask_tmp.mnc", "1", "26", "3"]);
    Spawn(["mv", "-f", "${TmpDir}/cortical_mask_tmp.mnc", $Mask]);
...
Spawn(["pve", "-image", $input_volume, "-curve", "${TmpDir}/curve_cg.mnc", "-mask", $Mask, $t1_volume, "${TmpDir}/pve"]);
Spawn(["skel", "${TmpDir}/pve_csf.mnc", $skelCSF]);
Spawn(["discretize_pve", "${TmpDir}/pve_csf.mnc", "${TmpDir}/pve_wm.mnc", "${TmpDir}/pve_gm.mnc", $cls_correct]);
...
Spawn(["expand_from_white", $input_volume, "${output_prefix}_white_81920.obj", "${output_prefix}_gray_81920.obj", $laplace]);
