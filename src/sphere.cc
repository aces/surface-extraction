...
void print_flag( char * msg, int flag ) {

  printf( "%s ", msg );
  if( flag & FACE0 ) printf( "FACE0 " );
  if( flag & FACE1 ) printf( "FACE1 " );
  if( flag & FACE2 ) printf( "FACE2 " );
  if( flag & FACE3 ) printf( "FACE3 " );
  if( flag & FACE4 ) printf( "FACE4 " );
  if( flag & FACE5 ) printf( "FACE5 " );
  if( flag & EDGE0 ) printf( "EDGE0 " );
  if( flag & EDGE1 ) printf( "EDGE1 " );
  if( flag & EDGE2 ) printf( "EDGE2 " );
  if( flag & EDGE3 ) printf( "EDGE3 " );
  if( flag & EDGE4 ) printf( "EDGE4 " );
  if( flag & EDGE5 ) printf( "EDGE5 " );
  if( flag & EDGE6 ) printf( "EDGE6 " );
  if( flag & EDGE7 ) printf( "EDGE7 " );
  if( flag & EDGE8 ) printf( "EDGE8 " );
  if( flag & EDGE9 ) printf( "EDGE9 " );
  if( flag & EDGE10 ) printf( "EDGE10 " );
  if( flag & EDGE11 ) printf( "EDGE11 " );
  if( flag & NODE0 ) printf( "NODE0 " );
  if( flag & NODE1 ) printf( "NODE1 " );
  if( flag & NODE2 ) printf( "NODE2 " );
  if( flag & NODE3 ) printf( "NODE3 " );
  if( flag & NODE4 ) printf( "NODE4 " );
  if( flag & NODE5 ) printf( "NODE5 " );
  if( flag & NODE6 ) printf( "NODE6 " );
  if( flag & NODE7 ) printf( "NODE7 " );
  printf( "\n" );
}
...
int make_sphere( int sizes[MAX_DIMENSIONS], short * val ) {
    ...
    short * levels = new short[n_voxels];
    if( !levels ) {
...
void save_voxel_surface( int sizes[3], Real dh[3], short * val,
                         Volume volume, char * obj_file, int subsample ) {
    ...
    int * n_ngh = new int[count];
    if( !n_ngh ) {
      printf( "Error allocating memory for n_ngh.\n" );
      exit( 1 );
    }
    int * ngh = new int[3*count];
    if( !ngh ) {
    ...
    // Must do refinement by hand since Volume doesn't know about it.
    if( subsample ) {
      voxel_000[0] *= 2;
      voxel_000[1] *= 2;
      voxel_000[2] *= 2;
    ...
    float * new_coords = new float[3*count];
    if( !new_coords ) {
...
int surface( int sizes[MAX_DIMENSIONS], Real dh[MAX_DIMENSIONS],
             Volume mask, char * obj_file, int subsample ) {
    ...
    if( subsample ) {
      printf( "Doing voxel refinement...\n" );

      short * big = new short[8*sizes[0]*sizes[1]*sizes[2]];
      if( !big ) {
        printf( "Error allocating memory for big.\n" );
        exit( 1 );
      }

      count = 0;
      for( int i = 0;  i < 2*sizes[0];  i++ ) {
        for( int j = 0;  j < 2*sizes[1];  j++ ) {
          for( int k = 0;  k < 2*sizes[2];  k++ ) {
            ii = (i/2) * sizes[1] * sizes[2] + (j/2) * sizes[2] + (k/2);
            big[count] = val[ii];
            count++;
          }
        }
...
int  main( int ac, char* av[] ) {
    ...
    if( ac < 3 ) {
      cerr << "Usage: " << av[0] << " mask.mnc output.obj [-subsample]" 
           << endl << endl
           << "Copyright Alan C. Evans" << endl
    ...
    int subsample = 0;
    if( ac == 4 ) {
      if( strcmp( av[3], "-subsample" ) == 0 ) subsample = 1;
    }
    ...
    if ( input_volume( av[1], 3, NULL, 
      ...
      cerr << "Error: cannot read white matter volume file " << av[1] << endl;
      return 1;
    }
